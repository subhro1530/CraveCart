...
export default function AccountPanel({ user, addresses, orders, error, pending, onClose, onSignOut, onProfileSave, onAddressSave, onAddressUpdate, onAddressDelete, onOrderCancel }) {
  ...
  return <div className="dialog-backdrop"><section className="dialog account-dialog" role="dialog" aria-modal="true" aria-labelledby="account-title">
    ...
    <div className="account-list"><h3>Previous orders</h3>{orders.length ? orders.map((order) => <div className="account-row" key={order.id}><span><strong>Order #{order.id}</strong> · <b>{(order.status || 'Placed').replaceAll('_', ' ')}</b><br /><small>{order.fulfillment_type === 'PICKUP' ? 'Scheduled van pickup' : 'Delivery'} · {order.created_at ? new Date(order.created_at).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' }) : 'Time pending'}</small></span><span>${Number(order.total || 0).toFixed(2)} {['PENDING', 'CONFIRMED'].includes(order.status) && <button className="text-button" type="button" onClick={() => onOrderCancel(order.id)}>Cancel</button>}</span></div>) : <p className="account-note">No orders yet.</p>}</div>
